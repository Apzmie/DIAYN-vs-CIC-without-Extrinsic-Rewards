...
class PolicyNetwork(nn.Module):
    ...
    def sample(self, x, goal):
        mean, log_std = self.forward(x, goal)
        std = log_std.exp()
    ...
    def deterministic(self, x, goal):
        mean, _ = self.forward(x, goal)
...
class ReplayBuffer:
    def __init__(self, state_dim, action_dim, goal_dim, max_size=int(1e6), batch_size=256):
...
class SACAgent:
    def __init__(self, state_dim, goal_dim, action_dim, lr=3e-4):
        self.actor = PolicyNetwork(state_dim, goal_dim, action_dim)
        self.critic1 = QNetwork(state_dim, goal_dim, action_dim)
        self.critic2 = QNetwork(state_dim, goal_dim, action_dim)
        self.critic1_target = QNetwork(state_dim, goal_dim, action_dim)
        self.critic2_target = QNetwork(state_dim, goal_dim, action_dim)
    ...
    def update(self, batch):
        ...
        goal = torch.FloatTensor(batch['goal'])
        ...
        with torch.no_grad():
            ...
            next_q1 = self.critic1_target(next_state, goal, next_action)
            next_q2 = self.critic2_target(next_state, goal, next_action)
            next_q = torch.min(next_q1, next_q2)
            
            alpha = self.log_alpha.exp()            
            target_q = reward + self.gamma * (1 - done) * (next_q - alpha * next_log_prob)
            
        q1 = self.critic1(state, goal, action)
        q2 = self.critic2(state, goal, action)
        ...
        action_new, log_prob = self.actor.sample(state, goal)
        
        q1_new = self.critic1(state, goal, action_new)
        q2_new = self.critic2(state, goal, action_new)
...
def load_checkpoint(path, agent, buffer):
    ...
    buffer.goal = ckpt["replay_buffer"]["goal"]
...
if __name__ == "__main__":
    ...
    goal_dim = ?
    agent = SACAgent(state_dim, goal_dim, action_dim)
    buffer = ReplayBuffer(state_dim, goal_dim, action_dim)
    ...
    test_max_step = 1000
    distance_threshold = ?
    goal = torch.tensor([?, ?], dtype=torch.float32)
    ...
    while True:
        ...
        if len(agent_ids) > 0:
            ...
            goal_batch = goal.repeat(len(agent_ids), 1)
            ...
            if total_steps < random_exploration_steps:
                ...
                with torch.no_grad():
                    actions, _ = agent.actor.sample(states_tensor, goal_batch)   
        ...
        for i, agent_id in enumerate(agent_ids):
            ...
            if agent_id not in one_episode:
                one_episode[agent_id] = []
            
            one_episode[agent_id].append((state, action, reward, next_state, done, goal.numpy()))
            if done:
                traj = one_episode[agent_id]
                T = len(traj)
                for t in range(T):
                    transition = traj[t]
                    buffer.add(*transition)
                    
                    future = random.randint(t, T-1)
                    future_state = traj[future][0]
                    new_goal = future_state[-goal_dim:]
                    
                    (state, action, reward, next_state, done, goal) = transition
                    reached = next_state[-goal_dim:]
                    distance = np.linalg.norm(reached - new_goal)
                    new_reward = 1.0 if distance < distance_threshold else 0.0
                    buffer.add(state, action, new_reward, next_state, done, new_goal)
